...
function angleBetweenVectors_2(v1, v2) {
  // Calculate dot product
  const dotProduct = v1.x * v2.x + v1.y * v2.z;
...
export function step(RADIUS, sceneEntities, world, scene, customParams = {}) {
  ...
  let prev_pos = new THREE.Vector3();
  let prev_pos2=[];
...
//----------------------------------------------------  SPECIAL CASE CODE -- END ------------------------------------------------------------------------------------------
  ...
  function longRangeConstraintCapsule(best_i, best_j,
    ...
    if (d_sq > 0.0 && Math.abs(a) > epsilon && tao > 0 && tao < C_TAU_MAX) {
      ...
      // if (bestPointDiff <= 0.01 &&  (facingDiff < 0.01 || facingDiff - Math.PI) <0.01 ){
      // if (bestPointDiff <= 2.0 &&  (facingDiff < 0.1 || facingDiff - Math.PI) <0.1 ){
        if (bestPointDiff <= 4.0 &&  (facingDiff < 0.015 || facingDiff - Math.PI) <0.015 ){
...
// -------------- Start --------------------- for calculating normal to capsule body ----------------------
    ...
    // let velocityDir = Math.atan2(sceneEntities[i].vx, sceneEntities[i].vy, sceneEntities[i].vz )*180/Math.PI;
    // console.log("velocityDir: ", velocityDir);

    let current_pos = new THREE.Vector3(sceneEntities[i].x, sceneEntities[i].y, sceneEntities[i].z);
    let predicted_pos = new THREE.Vector3(sceneEntities[i].px, sceneEntities[i].py, sceneEntities[i].pz);

    let velocityDir = new THREE.Vector3();
    // velocityDir.subVectors(sceneEntities[i].current_pos, sceneEntities[i].predicted_pos).normalize();
    // console.log(" predicted_pos: ", predicted_pos);
    ...
    // Generate a normal vector (-dy, dx) 
    // This will be perpendicular to the line
    const nx_b = -dz_b / leng_b;  
    const nz_b = dx_b / leng_b;

    // Normal vector
    const normal_to_capsule_b = {x: nx_b, y:0 , z: nz_b};
    sceneEntities[j].normal_to_capsule = normal_to_capsule_b;

    let velocityVector_b = { x: sceneEntities[j].vx, y: sceneEntities[j].vz }; // Velocity vector of the agent

    // sceneEntities[j].agent_velocity = new THREE.Vector3 { x: sceneEntities[j].vx, y: 0, z: sceneEntities[j].vz };
    sceneEntities[j].agent_velocity = new THREE.Vector3 (sceneEntities[j].vx, 0, sceneEntities[j].vz );

    let angle_4_b = angleBetweenVectors_2(velocityVector_b, normal_to_capsule_b);
...
// ------------------ END ----------------- for calculating normal to capsule body ----------------------
    ...
    const dx_1 = sceneEntities[i].vx - sceneEntities[i].normal_to_capsule.x;  // px -> v, x->normal
    const dz_2 = sceneEntities[i].vz - sceneEntities[i].normal_to_capsule.z;
    // sceneEntities[i].agent.rotation.z = Math.atan2(dz_2, dx_1); 

    // const dx_3 = sceneEntities[j].vx - sceneEntities[j].normal_to_capsule.x;  // px -> v, x->normal
    // const dz_4 = sceneEntities[j].vz - sceneEntities[j].normal_to_capsule.z;
    // sceneEntities[j].agent.rotation.z = Math.atan2(dz_4, dx_3); 
...
/*
        const dx_5 = sceneEntities[i].px - sceneEntities[i].x;  
        const dz_5 = sceneEntities[i].pz - sceneEntities[i].z;
        sceneEntities[i].agent.rotation.z = Math.atan2(dz_5, dx_5); 

        const dx6 = sceneEntities[j].px - sceneEntities[j].x;  
        const dz6 = sceneEntities[j].pz - sceneEntities[j].z;
        sceneEntities[j].agent.rotation.z =  Math.atan2(dz6, dx6); 
    */    

        // sceneEntities[j].agent.translateZ( 0.01 );

        // sceneEntities[i].agent.lookAt(new THREE.Vector3(sceneEntities[i].px, sceneEntities[i].py, sceneEntities[i].pz) );
        // sceneEntities[j].agent.lookAt(new THREE.Vector3(sceneEntities[j].px, sceneEntities[j].py, sceneEntities[j].pz) );
        ...
        aimP.copy(current_pos).add(velocityVector_i);
        sceneEntities[i].agent.lookAt(aimP);
        */

        
        // const dx_5 = sceneEntities[i].x - delta_correction_i.x;  
        // const dz_5 = sceneEntities[i].z - delta_correction_i.y;
        // sceneEntities[i].agent.rotation.z = Math.atan2(dz_5, dx_5);

        // const dx6 = sceneEntities[j].x - delta_correction_j.x;  
        // const dz6 = sceneEntities[j].z - delta_correction_j.y ;
        // sceneEntities[j].agent.rotation.z = Math.atan2(dz6, dx6);
...
/*
        ...
        sceneEntities[j].prev_rotation_angle_rad = sceneEntities[j].cur_rotation_angle_rad;
        sceneEntities[j].cur_rotation_angle_rad = Math.atan2(dz6, dx6);

        sceneEntities[j].prev_rotation_angle_deg = sceneEntities[j].cur_rotation_angle_deg;
        sceneEntities[j].cur_rotation_angle_deg = Math.atan2(dz6, dx6) * (180/3.1416);
*/

        // if( Math.abs(sceneEntities[j].cur_rotation_angle_deg - sceneEntities[j].prev_rotation_angle_deg) < 5 )
        // {
        //   console.log(" prev: ", sceneEntities[j].prev_rotation_angle_deg, ", cur: ", sceneEntities[j].cur_rotation_angle_deg, ", diff: ", sceneEntities[j].cur_rotation_angle_deg - sceneEntities[j].prev_rotation_angle_deg );
        //   sceneEntities[j].agent.rotation.z =  Math.atan2(dz6, dx6); 
        //   sceneEntities[j].last_used_rotation_angle_rad = sceneEntities[j].agent.rotation.z;
        // }else{
        //   sceneEntities[j].agent.rotation.z =  sceneEntities[j].last_used_rotation_angle_rad; 
        // }

/*
        // if( Math.abs(sceneEntities[j].cur_rotation_angle_deg - sceneEntities[j].prev_rotation_angle_deg) > 5 )
        if( Math.abs(sceneEntities[j].cur_rotation_angle_rad - sceneEntities[j].prev_rotation_angle_rad) > .1 )
        {

          sceneEntities[j].agent.rotation.z =  sceneEntities[j].prev_rotation_angle_rad + sceneEntities[j].cur_rotation_angle_rad/10; 
          sceneEntities[j].last_used_rotation_angle_rad = sceneEntities[j].agent.rotation.z;

        for (let i = 0; i <= sceneEntities[j].cur_rotation_angle_rad; i++) 
        {
            // sceneEntities[j].agent.rotation.z = sceneEntities[j].prev_rotation_angle_rad + ( Math.atan2(dz6, dx6)/1000);
            sceneEntities[j].agent.rotation.z = sceneEntities[j].prev_rotation_angle_rad + 0.000018;
            console.log("if executed");
        }


        // sceneEntities[j].agent.rotation.z = sceneEntities[j].prev_rotation_angle_rad + 0.00045;
        // console.log("sceneEntities[j].prev_rotation_angle_rad: ", sceneEntities[j].prev_rotation_angle_rad);
...
/*
        const dx5 = sceneEntities[i].px - sceneEntities[i].x;  
        const dz5 = sceneEntities[i].pz - sceneEntities[i].z;
        // if(dz5 > dx5)
        // {
        //   sceneEntities[i].agent.rotation.z = Math.atan2(dz5, dx5);
        // }
        sceneEntities[i].agent.rotation.z = Math.atan2(dz5, dx5) * timestep;
        ...
        // if(dz6 > dx6)
        // {
        //   sceneEntities[i].agent.rotation.z = Math.atan2(dz6, dx6);
        // }
        sceneEntities[j].agent.rotation.z = Math.atan2(dz6, dx6) * timestep; 
        console.log("dx_5: ", dz5, ", dz_5: ", dz5, ", radians: ", Math.atan2(dz5, dx5));
*/        


      // const dx_1 = sceneEntities[i].vx - sceneEntities[i].normal_to_capsule.x;  
      // const dz_2 = sceneEntities[i].vz - sceneEntities[i].normal_to_capsule.z;
      // sceneEntities[i].agent.rotation.z = Math.atan2(dz_2, dx_1); 

      // const dx_3 = sceneEntities[j].vx - sceneEntities[j].normal_to_capsule.x;  
      // const dz_4 = sceneEntities[j].vz - sceneEntities[j].normal_to_capsule.z;
      // sceneEntities[j].agent.rotation.z = Math.atan2(dz_4, dx_3); 
...
//******************************** START - With prev position and  current position *************************************
/*
let cur_pos = new THREE.Vector3(item.x, item.y, item.z);
// const prev_pos = new THREE.Vector3();
console.log("cur_pos: ", cur_pos, ", prev_pos2: ", prev_pos2);
// console.log("prev_pos2: ", prev_pos2);
// prev_pos.copy(cur_pos);
prev_pos2.push(cur_pos);
*/

    // const dx = item.x - item.x_prev; 
    // const dz = item.z - item.z_prev;;
    // item.agent.rotation.z = Math.atan2(dz, dx); 


// const dx = item.x_prev - item.x_2nd_prev; 
// const dz = item.z_prev - item.z_2nd_prev;
// item.agent.rotation.z = Math.atan2(dz, dx); 

// item.x_2nd_prev = item.x_prev;
// item.z_2nd_prev = item.z_prev;
...
// item.cur_rotation_angle_deg = Math.atan2(dz, dx) * (180/3.1416);

// item.prev_rotation_angle_rad = item.cur_rotation_angle_rad;
// item.cur_rotation_angle_rad = Math.atan2(dz, dx);

// if( item.cur_rotation_angle - item.prev_rotation_angle < 1)
// {
//   item.agent.rotation.z = Math.atan2(dz, dx);
//   console.log("prev rotat: ", item.prev_rotation_angle, ", cur_rotat: ", item.cur_rotation_angle );
// }

// if( item.cur_rotation_angle_deg > item.prev_rotation_angle_deg )
// {
//   item.agent.rotation.z = Math.atan2(dz, dx);
//   // console.log("prev rotat: ", item.prev_rotation_angle_deg, ", cur_rotat: ", item.cur_rotation_angle_deg );
//   // console.log("if executed");
// }
// else{
//   item.agent.rotation.z = item.prev_rotation_angle_rad;
...
// console.log("prev rotat: ", item.prev_rotation_angle_deg, ", cur_rotat: ", item.cur_rotation_angle_deg );
  ...
  // if( item.cur_rotation_angle_rad > 0 )
  // {
  //   item.agent.rotation.z = item.prev_rotation_angle_rad + 0.0001;
  //   // console.log("If executed");
  // }
  // else{
  //   item.agent.rotation.z = item.prev_rotation_angle_rad - 0.0001;
...
//******************************** End - With prev position and  current position ***************************************



 /*  
    let p_x = item.px - item.x;
    let p_z = item.pz - item.z;

    let vec_pos = new THREE.Vector3(p_x, 0, p_z);

    let vec_vel = new THREE.Vector3(item.vx, 0, item.vz);

    let vec_cur_pos = new THREE.Vector3(item.x, 0, item.z);

    // Normalize the velocity vector to get the direction vector
    var dir = vec_vel.clone().normalize();
    console.log("dir: ", dir);
    
    //Calculate the magnitude of v:
    let mag = Math.sqrt(item.vx * item.vx + item.vy * item.vy + item.vz * item.vz);

    let newDir = new THREE.Vector3();

    // Scale dir by the desired distance d and the magnitude mag:
    // newDir = dir.multiplyScalar(0.05 * mag);
    newDir = dir.multiplyScalar(0.05).multiplyScalar(mag);
    // let newDir = dir.multiply(d).multiply(mag)

    const newPoint = new THREE.Vector3();
    newPoint.addVectors( vec_cur_pos, newDir );

    item.new_proj_pos = newPoint;

    const new_projected_pos = new THREE.Vector2(item.new_proj_pos.x, item.new_proj_pos.z);
    const current_pos = new THREE.Vector2(item.x, item.z);
    const predicted_pos = new THREE.Vector2(item.px, item.pz);

    // let dis = distance(item.new_proj_pos.x, item.new_proj_pos.z, item.x, item.z);
    // let dis = distance(item.px, item.pz, item.x, item.z);
    // console.log("dis: ", dis);
*/


    let predicted_pos = new THREE.Vector3(item.px, item.py, item.pz);  
    // let vec_vel = new THREE.Vector3(item.vx, 0, item.vz);
    let vec_cur_pos = new THREE.Vector3(item.x, item.y, item.z);
    let vec_vel = predicted_pos.clone().sub(vec_cur_pos);
    // console.log("vec_vel: ", vec_vel);
    // Normalize the velocity vector to get the direction vector
    let dir = vec_vel.clone().normalize();
    let desiredDistance = 2; 
    // Calculate displacement vector
    let displacement = dir.clone().multiplyScalar(desiredDistance);
    // Calculate new point
    var newPosition = vec_cur_pos.clone().add(displacement);

    item.new_proj_pos = newPosition;

    
    // const dx = newPoint.getComponent(0) - item.x;  // px -> v, x->normal
    // const dz = newPoint.getComponent(2) - item.z;
    // item.agent.rotation.z = Math.atan2(dz, dx); 

    // const dx = item.new_proj_pos.x - item.x; 
    // const dz = item.new_proj_pos.z - item.z;
    // item.agent.rotation.z = Math.atan2(dz, dx); 


    // const dx8 = item.px - item.x;  
    // const dz8 = item.pz - item.z;
    // item.agent.rotation.z = Math.atan2(dz8, dx8); 


    // const dx = item.px - item.x;  
    // const dz = item.pz - item.z;
    // let angle_allowed = Math.atan2(dz, dx); 
    // if(angle_allowed > 0.437)
    // {
    //   item.agent.rotation.z = 0.437;
    // }else if(angle_allowed < -0.437)
    // {
    //   item.agent.rotation.z = -0.437;
    // }
    ...
    // const dx = item.vx - item.normal_to_capsule.x;  // px -> v, x->normal
    // const dz = item.vz - item.normal_to_capsule.z;
    // item.agent.rotation.z = Math.atan2(dz, dx); 
