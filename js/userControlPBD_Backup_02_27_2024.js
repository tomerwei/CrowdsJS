...
// Function to find a vector that is theta degree away from a known direction vector
function findVectorAwayFromDirection(knownVector, theta) {
  // Normalize the known vector to ensure it's a unit vector
  // knownVector.normalize();

  // Define an axis perpendicular to the known vector
  var perpendicularAxis = new THREE.Vector3();
  perpendicularAxis.crossVectors(knownVector, new THREE.Vector3(1, 0, 0));
  // If the cross product yields a zero vector, use a different axis
  if (perpendicularAxis.lengthSq() === 0) {
      perpendicularAxis.crossVectors(knownVector, new THREE.Vector3(0, 1, 0));
  }

  // Apply rotation around the perpendicular axis by theta degrees
  var newVector = knownVector.clone().applyAxisAngle(perpendicularAxis, theta);
...
function findNewPointInVectorDirectionWithAngle(point, vector, angle) {
  ...
  // var angle = scalarProjection / (Math.sqrt(point.x * point.x + 0 + point.z * point.z) * Math.sqrt(vector.x * vector.x + 0 + vector.z * vector.z));
  // console.log("cosAngle: ", cosAngle*(180/Math.PI));
...
function findPointsDeviatedByAngle_and_certain_dist(currPos, predictedPos, unitDir, theta, distance) {
  // Calculate the vector from the current position to the predicted position
const movementVector = predictedPos.clone().sub(currPos);

// Calculate the projection of the movement vector onto the unit vector direction
const scalarProjection = movementVector.dot(unitDir);
const projectedVector = unitDir.clone().multiplyScalar(scalarProjection);

// Calculate the vector perpendicular to the unit vector direction
const perpVector = movementVector.clone().sub(projectedVector).normalize();

// Calculate the new position
const newPos = currPos.clone().add(projectedVector.multiplyScalar(distance * Math.cos(theta))).add(perpVector.multiplyScalar(distance * Math.sin(theta)));
...
//----------------------------------------------------  SPECIAL CASE CODE -- END ------------------------------------------------------------------------------------------
  ...
  function longRangeConstraintCapsule_by_prof(best_i, best_j,
    p_best_i, p_best_j,
    theta_i, theta_j,
    agent_i, agent_j,
    entity_i, entity_j,
...
i = 0;
    while (i < sceneEntities.length) {
      ...
      while (j < sceneEntities.length) {
        ...
        let [delta_correction_i, delta_correction_j, grad_i, grad_j, s] = longRangeConstraintCapsule_by_prof(
            bestA, bestB,
            p_bestA, p_bestB,
...
// -------------- Start --------------------- for calculating normal to capsule body ----------------------
    ...
    sceneEntities[i].normal_to_capsule = normal_to_capsule;
    // console.log(normal_to_capsule)

  /*
    // Not sure calculating the angle between velocity and capsule normal to body would work here or not as vx and vz haven't been updated here yet. 
    //Only new position corrections have been  calculated followed by calculating px and pz. 
    let velocityVector = { x: sceneEntities[i].vx, y:0, z: sceneEntities[i].vz }; // Velocity vector of the agent
    sceneEntities[i].agent_velocity = { x: sceneEntities[i].vx, y: 0, z: sceneEntities[i].vz };
    let angle_4 = angleBetweenVectors_2(velocityVector, normal_to_capsule);
    let angle_5 = degreeBetween(sceneEntities[i].vx, sceneEntities[i].vz, sceneEntities[i].normal_to_capsule.x, sceneEntities[i].normal_to_capsule.z);
  */
...
/*
  ...
  */
    ...
    if(angle_6 > angle_threshold && sceneEntities[i].x < 20 )
    {
      ...
      // console.log("new_point_left: ", new_point_left, "\n\n");
...
/*
    if(angle_6 > 2)
    {
      console.log("angle_6 before : ", angle_6)
      let dist_between_cur_and_predict = distance(sceneEntities[i].x, sceneEntities[i].z, sceneEntities[i].px, sceneEntities[i].pz);
      let AB = dist_between_cur_and_predict * Math.cos( (angle_6- 2) * (Math.PI/180) );

      let AC = AB * Math.cos(2 * (Math.PI/180) );
      let BC = AB * Math.sin(2 * (Math.PI/180) );
      // console.log("AC: ", AC, ", BC: ", BC);

      // console.log("Before, px: ", sceneEntities[i].px, ", pz: ", sceneEntities[i].pz);
      sceneEntities[i].px = sceneEntities[i].x + AC
      sceneEntities[i].pz = sceneEntities[i].z + BC;
      // console.log("AC: ", AC, ", BC: ", BC);
      // console.log("After, px: ", sceneEntities[i].px, ", pz: ", sceneEntities[i].pz);
      // console.log("\n");
...
/*
    //----------------- pri ------- start ------------------------------------------------
    var theta =2;
    if(angle_6 > theta && sceneEntities[i].x < 20)
    { 
    let dist_between_cur_and_predict = distance(sceneEntities[i].x, sceneEntities[i].z, sceneEntities[i].px, sceneEntities[i].pz);
      
    //checking current angle (before fix)
    var currentPosition = new THREE.Vector3(sceneEntities[i].x, 2, sceneEntities[i].z);
    var predicted_position = new THREE.Vector3(sceneEntities[i].px, 2, sceneEntities[i].pz);
    var displacementVector = predicted_position.clone().sub(currentPosition);
    var directionOfMovement = displacementVector.clone().normalize();
    var directionVector = new THREE.Vector3(directionOfMovement.x, 2, directionOfMovement.z);
    let angle_before = angleBetweenVectors_2(directionVector, normal_to_capsule);
    console.log("angleA before : ", currentPosition, predicted_position, displacementVector, directionVector, angle_before)


   
    //move to a point on a line which is in theta angle with respect to normal
    var normaPositionVector = new THREE.Vector3(normal_to_capsule.x, normal_to_capsule.y, normal_to_capsule.z);
    var axis = new THREE.Vector3(1, 0, 0);
    var angle = theta * Math.PI / 180.0;
    var targetVector = normaPositionVector.clone().applyAxisAngle(axis, angle)

    var direction = new THREE.Vector3().copy(targetVector).normalize();
    var scaledDirection = direction.multiplyScalar(dist_between_cur_and_predict);
    var newPoint = new THREE.Vector3().copy(currentPosition).add(scaledDirection);

    sceneEntities[i].px = newPoint.x;
    // sceneEntities[i].py = newPoint.y;
    sceneEntities[i].pz = newPoint.z;
    
    // previous codes
    // var phi = theta - angle_6;
    // var phiRad = theta * Math.PI / 180.0
    // console.log("phiRad ", phiRad);
    // sceneEntities[i].px = sceneEntities[i].px - dist_between_cur_and_predict * Math.cos(phiRad);
    // sceneEntities[i].pz = sceneEntities[i].pz - dist_between_cur_and_predict * Math.sin(phiRad);
    ...
    let angle_after = angleBetweenVectors_2(directionVector, normal_to_capsule);
    console.log("angleA after : ", currentPosition, predicted_position, displacementVector, directionVector, angle_after)
    }
    //----------------- pri ------- end ------------------------------------------------
*/
    ...
    // let velocityVector_i = new THREE.Vector3 (sceneEntities[i].vx, 0, sceneEntities[i].vz) ; // Velocity vector of the agent
    // let velocityVector_i_norm = velocityVector_i.normalize();
    // sceneEntities[i].velocityVector_i_norm = velocityVector_i_norm;

//------------------------------
    /*
    // Not sure calculating the angle between velocity and capsule normal to body would work here or not as vx and vz haven't been updated here yet. 
    //Only new position corrections have been  calculated followed by calculating px and pz. 
    let velocityVector = { x: sceneEntities[j].vx, y:0, z: sceneEntities[j].vz }; // Velocity vector of the agent
    sceneEntities[i].agent_velocity = { x: sceneEntities[j].vx, y: 0, z: sceneEntities[j].vz };
    let angle_4_b = angleBetweenVectors_2(velocityVector, normal_to_capsule_b);
    let angle_5_b = degreeBetween(sceneEntities[j].vx, sceneEntities[j].vz, sceneEntities[j].normal_to_capsule_b.x, sceneEntities[j].normal_to_capsule_b.z);
    */
...
/*
    ...
    if(angle_4_b > theta )
    { 
    let dist_between_cur_and_predict = distance(sceneEntities[j].x, sceneEntities[j].z, sceneEntities[j].px, sceneEntities[j].pz);
      
    //checking current angle (before fix)
    var currentPosition = new THREE.Vector3(sceneEntities[j].x, 2, sceneEntities[j].z);
    var predicted_position = new THREE.Vector3(sceneEntities[j].px, 2, sceneEntities[j].pz);
    var displacementVector = predicted_position.clone().sub(currentPosition);
    var directionOfMovement = displacementVector.clone().normalize();
    var directionVector = new THREE.Vector3(directionOfMovement.x, 2, directionOfMovement.z);
    let angle_before = angleBetweenVectors_2(directionVector, normal_to_capsule_b);
    console.log("angleA before : ", currentPosition, predicted_position, displacementVector, directionVector, angle_before)


   
    //move to a point on a line which is in theta angle with respect to normal
    var normaPositionVector = new THREE.Vector3(normal_to_capsule_b.x, normal_to_capsule_b.y, normal_to_capsule_b.z);
    var axis = new THREE.Vector3(1, 0, 0);
    var angle = theta * Math.PI / 180.0;
    var targetVector = normaPositionVector.clone().applyAxisAngle(axis, angle)
    ...
    sceneEntities[j].px = newPoint.x;
    sceneEntities[j].py = newPoint.y;
    sceneEntities[j].pz = newPoint.z;
...
// ------------------ END ----------------- for calculating normal to capsule body ----------------------
    ...
    // const dx_1 = sceneEntities[i].velocityVector_i_norm.x - sceneEntities[i].normal_to_capsule.x;  // px -> v, x->normal
    // const dz_2 = sceneEntities[i].velocityVector_i_norm.z - sceneEntities[i].normal_to_capsule.z;
    // sceneEntities[i].agent.rotation.z = Math.atan2(dz_2, dx_1); 

    /*
//********************************** averaging the last few rotation - start ************** ************** ************** 
        const dx12 = sceneEntities[i].px - sceneEntities[i].x;
        const dz12 = sceneEntities[i].pz - sceneEntities[i].z;

        sceneEntities[i].cur_rotation_angle_rad = Math.atan2(dz12, dx12);
        // item.agent.rotation.z = Math.atan2(dz, dx); 

        sceneEntities[i].last_ten_rotations.push(sceneEntities[i].cur_rotation_angle_rad); // Add current position to positions array
        if (sceneEntities[i].last_ten_rotations.length > 3) {
          sceneEntities[i].last_ten_rotations.shift();
        }
        // console.log("item.last_ten_rotations: ", item.last_ten_rotations);
        var sum = sceneEntities[i].last_ten_rotations.reduce(function(a, b) {
          return a + b;
        }, 0);

        // Calculate the average by dividing the sum by the number of elements
        var average = sum / sceneEntities[i].last_ten_rotations.length;
        sceneEntities[i].agent.rotation.z = average;
        //---------------------------------------------------------------------------

        const dx13 = sceneEntities[j].px - sceneEntities[j].x;
        const dz13 = sceneEntities[j].pz - sceneEntities[j].z;

        sceneEntities[j].cur_rotation_angle_rad = Math.atan2(dz13, dx13);
        // item.agent.rotation.z = Math.atan2(dz, dx); 

        sceneEntities[j].last_ten_rotations.push(sceneEntities[j].cur_rotation_angle_rad); // Add current position to positions array
        if (sceneEntities[j].last_ten_rotations.length > 3) {
          sceneEntities[j].last_ten_rotations.shift();
        }
        // console.log("item.last_ten_rotations: ", item.last_ten_rotations);
        var sum = sceneEntities[j].last_ten_rotations.reduce(function(a, b) {
          return a + b;
        }, 0);

        var average = sum / sceneEntities[j].last_ten_rotations.length;
        sceneEntities[j].agent.rotation.z = average;

        console.log("item.average: ", average);
//********************************** averaging the last few rotation - end ************** ************** ************** 
*/
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
/*
   var currentPosition = new THREE.Vector3(item.x, 0, item.z);
   var predicted_position = new THREE.Vector3(item.px, 0, item.pz);
   var displacementVector = predicted_position.clone().sub(currentPosition);
   var directionOfMovement = displacementVector.clone().normalize();
   var directionVector = new THREE.Vector3(directionOfMovement.x, directionOfMovement.y, directionOfMovement.z);
   //--------------------

    // // Calculate desired orientation based on velocity
    // var velocity = object.velocity; // Assuming you have velocity stored in each object
    // var direction = velocity.clone().normalize();

    // Calculate desired orientation quaternion
    // var targetQuaternion = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, 1), direction);
    // var targetQuaternion = new THREE.Quaternion().setFromUnitVectors(predicted_position, directionVector);
    var targetQuaternion = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, 1), directionVector);

    // Smoothly interpolate between current orientation and desired orientation using slerp
    var currentQuaternion = item.agent.quaternion.clone();
    var slerpAmount = 0.001; // Adjust this value as needed for the desired smoothness
    var newQuaternion = currentQuaternion.slerp(targetQuaternion, slerpAmount);

    // Apply the new orientation to the object
    item.agent.setRotationFromQuaternion(newQuaternion);
*/


 /*
    // Assuming you have current position vector and predicted position vector
    var currentPosition = new THREE.Vector3(item.x, 0, item.z);
    var predicted_position = new THREE.Vector3(item.px, 0, item.pz);
   ...
   // Normalize displacement vector to get direction of movement
   var directionOfMovement = displacementVector.clone().normalize();

   // Alternatively, you can create a new Three.Vector3 object for direction
   var directionVector = new THREE.Vector3(directionOfMovement.x, directionOfMovement.y, directionOfMovement.z);
  //  item.direction = directionVector;
  
   
//................. calculate direction from displacement ...........................
    // Assuming you have current position vector and predicted position vector
    var currentPosition = new THREE.Vector3(item.x, 0, item.z);
    var predicted_position = new THREE.Vector3(item.px, 0, item.pz);
   ...
   console.log("idx: ", item.index, ", item.direction: ", item.direction);
//..............................................................................................
 
   let vector_velocity = new THREE.Vector3(item.vx, 0, item.vz);
   // let vector_velocity_normalized = vector_velocity.normalize();
   let vector_velocity_normalized = vector_velocity.clone().normalize();
   // item.direction = new THREE.Vector3(vector_velocity_normalized.x, vector_velocity_normalized.y, vector_velocity_normalized.z);
   
   item.agent_position = currentPosition;

    
    const dx6 = item.px - item.x;  
    const dz6 = item.pz - item.z;
    item.agent.rotation.z =  Math.atan2(dz6, dx6); 


    // item.prev_rotation_angle_rad = item.cur_rotation_angle_rad;
    // item.cur_rotation_angle_rad = Math.atan2(dz6, dx6);

    // item.prev_rotation_angle_deg = item.cur_rotation_angle_deg;
    // item.cur_rotation_angle_deg = Math.atan2(dz6, dx6) * (180/3.1416);

/*
    if( Math.abs(item.cur_rotation_angle_deg - item.prev_rotation_angle_deg) < .5 )
    {
      // console.log(" prev: ", item.prev_rotation_angle_deg, ", cur: ", item.cur_rotation_angle_deg, ", diff: ", item.cur_rotation_angle_deg - item.prev_rotation_angle_deg );
      item.agent.rotation.z =  Math.atan2(dz6, dx6); 
      item.last_used_rotation_angle_rad = item.agent.rotation.z;
      console.log("if", item.agent.rotation.z);
    }else{
      item.agent.rotation.z =  item.last_used_rotation_angle_rad;
      item.last_used_rotation_angle_rad = item.agent.rotation.z; 
      console.log("else", item.agent.rotation.z);
    }
*/
...
/*
    // if( Math.abs(item.cur_rotation_angle_deg - item.prev_rotation_angle_deg) > 5 )
    if( Math.abs(item.cur_rotation_angle_rad - item.prev_rotation_angle_rad) > .1 )
    {
        // item.agent.rotation.z =  item.prev_rotation_angle_rad + item.cur_rotation_angle_rad/10; 
        // item.last_used_rotation_angle_rad = item.agent.rotation.z;

        for (let i = 0; i <= item.cur_rotation_angle_rad; i++) 
        {
            item.agent.rotation.z = item.prev_rotation_angle_rad + ( Math.atan2(dz6, dx6)/1000);
            item.prev_rotation_angle_rad = item.cur_rotation_angle_rad;
            // item.agent.rotation.z = item.prev_rotation_angle_rad + 0.000018;
            console.log("if executed", item.agent.rotation.z);
        }
        // item.agent.rotation.z = item.prev_rotation_angle_rad + 0.00045;
        // console.log("item.prev_rotation_angle_rad: ", item.prev_rotation_angle_rad);
    }else{
      item.agent.rotation.z = Math.atan2(dz6, dx6);
      item.prev_rotation_angle_rad = item.agent.rotation.z;
      console.log("else executed", item.agent.rotation.z);
    }
*/
    ...
    item.cur_rotation_angle_rad = Math.atan2(dz, dx);
    // item.agent.rotation.z = Math.atan2(dz, dx); 

    item.last_ten_rotations.push(item.cur_rotation_angle_rad); // Add current position to positions array
    if (item.last_ten_rotations.length > 150) {
      item.last_ten_rotations.shift();
    }
    // console.log("item.last_ten_rotations: ", item.last_ten_rotations);
    var sum = item.last_ten_rotations.reduce(function(a, b) {
      return a + b;
    }, 0);

    // Calculate the average by dividing the sum by the number of elements
    var average = sum / item.last_ten_rotations.length;
    item.agent.rotation.z = average; 
*/
