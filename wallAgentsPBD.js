...
export function step(RADIUS,sceneEntities,world) { 
	const WallsData = world['wallData'];
	const agentCount = world['agentCount'];
	const C_TAU_MAX = 20;
	...
	function resolveObstacleCollision(agent, obstacle) {
		...
		if (agent.x + RADIUS > obstacleX_TopLeft && agent.z + RADIUS > obstacleZ_TopLeft && agent.x - RADIUS < obstacleX_BottomRight && agent.z - RADIUS < obstacleZ_BottomRight) { //this if statement is never becoming true
			console.log("yes");
			normalDirX = (agent.x - (obstacleX_TopLeft + obstacleX_BottomRight)/2);
			normalDirZ = (agent.z - (obstacleZ_TopLeft + obstacleZ_BottomRight)/2);
	...
	function agentVelocityPlanner()  {
		sceneEntities.forEach(function (agent_i) {
			...
			if (agent_i.x < 5 && agent_i.x > -35 && agent_i.z < 5 && agent_i.z > -5) {
				agent_i.goal_x = goalPos2.x;
				agent_i.goal_z = agent_i.z;
			}
			if (agent_i.x <= -35) {
				if (agent_i.sFlag == 0) {
					agent_i.x = 39.5;
					agent_i.z = getRandomInt(-5, 5);
				}
				if (agent_i.sFlag == 1) {
					agent_i.x = getRandomInt(-5, 5);
					agent_i.z = -39.5;
					agent_i.goal_x = goalPos1;
					agent_i.goal_x = agent_i.x;
				}
			}			
		});
	}
